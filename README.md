# Hi, I'm Shubham Agrawal 👋

### AI/ML Engineer | Generative AI, LLMs & VLMs | Microservice Architecture

I am an AI/ML Engineer with a Post Graduate specialization in Artificial Intelligence and Machine Learning. I specialize in bridging the gap between core machine learning algorithms and modern generative AI technologies (LLMs and VLMs). My focus is on architecting intelligent, scalable backend systems and Human-in-the-Loop (HITL) workflows that solve complex domain-specific problems.

---

## 🚀 What I'm Currently Building

* **Decoupled STT & MRD Microservices:** Architecting a FastAPI monorepo containing isolated microservices for Speech-to-Text (STT) bifurcation and Medical Record Documentation (MRD).
* **AI-Driven Medical Compliance Engine:** Developing an automated verification system for medical documents (discharge summaries, consent forms) against NABH and IRDAI standards. This utilizes Gemini Vision for complex OCR tasks and strictly leverages external APIs for dynamic PM-JAY and ICD code mapping, avoiding rigid local database dependencies.
* **Legal RAG Chatbot:** Building a sophisticated Retrieval-Augmented Generation (RAG) pipeline designed to ingest, embed, and accurately query complex legal documents.
* **Algorithmic Stock Market Analysis:** Applying core machine learning algorithms and statistical forecasting to analyze market trends and identify quantitative trading patterns.

---

## 🛠️ Technical Stack & Expertise

| Category | Technologies & Concepts |
| :--- | :--- |
| **Machine Learning & AI** | Deep Learning, Statistical Analysis, Forecasting, Pattern Recognition. |
| **Generative AI** | LLMs, VLMs (Gemini Vision Pro), RAG Pipelines, Prompt Engineering, AI-driven OCR. |
| **Backend Architecture** | Python, FastAPI, Microservices, Asyncio, REST APIs, System Decoupling. |
| **Data Processing** | Multi-source ETL, Data Validation, PySpark. |

---

## 📈 Academic Background

* **Post Graduate in AI/ML** - Lambton College, Canada (2023 - 2025) 
* **B.C.A.** - Gujarat University (2018 - 2021) 

---

### 📫 Connect with Me:

[LinkedIn](www.linkedin.com/in/shubhamagrawal01) | [Email](SSAgrawal753@gmail.com)

> "Focusing on architectural elegance and data-driven decisions."
